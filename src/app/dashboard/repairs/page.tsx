...
import { Plus, Clock, Wrench, UserPlus, FileText, CheckCircle2, User, X, Briefcase, Edit2, Trash2, AlertTriangle, Loader2 } from 'lucide-react';
import { getMechanics, addMechanic, updateMechanic, deleteMechanic, addRepair } from '@/supabase/functions';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="inline-flex items-center gap-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-xl font-medium hover:bg-slate-50 transition-all shadow-sm"
          >
      ...
      <div className="grid gap-6">
        {mechanics.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
             ...
             <h3 className="text-lg font-semibold text-slate-900 mb-1">No hay mecánicos registrados</h3>
             <p className="text-slate-500 max-w-sm mx-auto mb-6">Agrega personal a tu equipo para empezar a registrar sus trabajos del día.</p>
             <button 
              onClick={() => {
                setMechanicName('');
                setIsAddMechanicOpen(true);
          ...
          mechanics.map(mechanic => {
            ...
            const totalDailyCost = mechanic.jobs.reduce((sum, job) => sum + job.cost, 0); // Assuming the query already filters for today or we show all recorded (user said "today's revenue")
                ...
                <div className="p-5 border-b border-slate-200 bg-slate-50/80 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 font-medium">{mechanic.jobs.length} trabajos hoy</p>
                    </div>
                  </div>
                  
                  <div className="flex items-center gap-4 w-full sm:w-auto mt-2 sm:mt-0">
                    <div className="bg-emerald-50 border border-emerald-100 px-4 py-2 rounded-xl text-right flex-1 sm:flex-none">
                      <p className="text-xs text-emerald-600 font-semibold uppercase tracking-wider">Producido Hoy</p>
                      <p className="text-lg font-bold text-emerald-700">$ {totalDailyCost.toLocaleString('es-CO')}</p>
                    </div>
                    <button 
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="text-xs text-slate-500 uppercase bg-white border-b border-slate-100">
                        <tr>
                          ...
                          <th className="px-6 py-4 font-semibold tracking-wider text-right w-48">Costo (COP)</th>
                        </tr>
                      </thead>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleLogJob} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-slate-700">Costo / Valor (COP)</label>
                  <div className="relative">
